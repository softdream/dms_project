...
public:
        ...
        bool initUdpServer( const int port = 2555 )
        {
...
public:
        ...
        bool recive( T& data )
        {
                ...
                memcpy( &data, this->recv_buffer, sizeof( data ) );
...
public:
	...
	template<typename T>
	void send2Warning( T& data ) 
	{
		char buffer[sizeof(data) + 4];
		buffer[0] = 's';
		buffer[1] = 'e';
		buffer[2] = 'l';
		buffer[3] = 'f';
		...
		this->write(buffer, sizeof(data) + 4, 2333);
		
		std::cout<<"send speed info to warning process ..."<<std::endl;
	}

	template<typename T>
	void send2MvAi( T& data ) 
	{
